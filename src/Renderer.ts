...
interface TileInfo {
  ...
  size: number;
  data?: any;
  layers?: { [key: string]: { scale: number; features: any[] } };
}
...
class Renderer {
  ...
  terminal!: { CLEAR: string; MOVE: string };
  tilePadding!: number;

  constructor(output: any, tileSource: any, style: any) {
    this.output = output;
  ...
  async draw(center: { lon: number; lat: number }, zoom: number): Promise<string | undefined> {
  ...
  private _visibleTiles(center: { lon: number; lat: number }, zoom: number): VisibleTile[] {
    const z = utils.baseZoom(zoom);
    const centerTile = utils.ll2tile(center.lon, center.lat, z);

    const tiles: VisibleTile[] = [];
    const tileSize = utils.tilesizeAtZoom(zoom);

    for (let y = Math.floor(centerTile.y) - 1; y <= Math.floor(centerTile.y) + 1; y++) {
      for (let x = Math.floor(centerTile.x) - 1; x <= Math.floor(centerTile.x) + 1; x++) {
        const tile = { x, y, z };
        const position = {
          x: this.width / 2 - (centerTile.x - tile.x) * tileSize,
          y: this.height / 2 - (centerTile.y - tile.y) * tileSize,
        };
  ...
  private async _getTile(tile: TileInfo): Promise<TileInfo> {
    tile.data = await this.tileSource.getTile(tile.xyz.z, tile.xyz.x, tile.xyz.y);
    return tile;
  }

  private _getTileFeatures(tile: TileInfo, zoom: number): TileInfo {
    const position = tile.position;
    const layers: { [key: string]: { scale: number; features: any[] } } = {};
  ...
  private async _renderTiles(tiles: VisibleTile[]): Promise<void> {
    const labels: { tile: VisibleTile; feature: any; scale: number }[] = [];
  ...
  featuresAt(x: number, y: number): void {
    this.labelBuffer.featuresAt(x, y);
  }

  private _drawFeature(tile: VisibleTile, feature: any, scale: number): boolean {
    let points: Point[], placed: boolean | undefined;
    ...
    switch (feature.style.type) {
      ...
      case 'fill': {
        points = feature.points.map((p: Point[]) => {
      ...
      case 'symbol': {
        ...
        if (this.seen[text] && !genericSymbol) {
          return false;
        }
  ...
  private _scaleAndReduce(tile: VisibleTile, feature: any, points: Point[], scale: number, filter: boolean = true): Point[] {
    let lastX: number | undefined;
    let lastY: number | undefined;
    let outside: boolean | null | undefined;
    const scaled: Point[] = [];
    ...
    if (feature.style.type !== 'symbol') {
      ...
      if (config.simplifyPolylines) {
        return simplify(scaled, .5, true) as Point[];
      } else {
        return scaled;
      }
    } else {
  ...
  private _generateDrawOrder(zoom: number): string[] {
    if (zoom < 2) {
      return [
        'admin',
        'water',
...
Renderer.prototype.labelBuffer = null as any;
Renderer.prototype.tileSource = null as any;
