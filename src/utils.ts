...
const utils = {
  ...
  tile2ll: (x: number, y: number, zoom: number): { lon: number; lat: number } => {
  ...
  normalize: (ll: { lon: number; lat: number }): { lon: number; lat: number } => {
